...
void DrawBeforeBoxClose3(void)
{
    glColor3ub(0,0,0);
    glRasterPos2d(100,80);
    YsGlDrawFontBitmap16x24("CHAOSSS!!");
}
...
int main(void)
{
    ...
    //printf("%1f\n",ystart[idx]-(hh*2.0));
    double xstart[3]={240.0,150.0,80.0};
    double ystart[3]={300.0,300.0,300.0};
    for (int i=0; i<size[idx]; ++i)
    {
    ...
    while(FSKEY_ESC!=FsInkey())
    {
        ...
        if (clip==1)
        {
            ...
            if (landed==0)
            {
                ...
                rocketflying.PlayOneShot(wavflying);
            }
            else
            {
                if (timecounter==0)
                {
                    FsPassedTime();
                    passed=0;//reset
                    //rocketflying.Stop(wavflying);
                    //rocketflying.End();
                    rocketlanding.PlayOneShot(wavlanding);
        ...
        else if (clip==3)
        {
            ...
            {
                //shuffling.PlayBackground(wavshuffling);
    ...
    //rocketflying.Stop(wavflying);
    //rocketlanding.Stop(wavlanding);
    //background.Stop(wavbackground);
